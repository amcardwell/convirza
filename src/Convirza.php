...
class Convirza
{
	...
	{
		if (!is_string($apikey)) {
			throw new ConvirzaException("Convirza API key is required - use the 'CONVIRZA_KEY' .env value");
		}
		if (is_null($api)) {
			$api = new ConvirzaApi($apikey);
		}
		$this->api = $api;
	}
	...
	public function status(string $listId, string $email): string
	{
		...
		if (!$this->memberStatusIsValid($member)) {
			throw new ConvirzaException('Unknown error, status value not found: ' . var_export($member, true));
		}
	...
	public function api(string $method, string $endpoint, array $data = []): array
	{
	     ...
	     protected function checkListExists(string $listId)
	     {
	     	try {
	     		...
	     		if ($this->api->responseCodeNotFound()) {
	     			throw new ConvirzaBadRequestException('Mailchimp API error: list id:'.$listId.' does not exist');
	     		}
	     		throw $e;
	     	}
	     }
